...
class FDAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.to_out(temporal_out), temporal_attn
...
class ViViTBackbone(nn.Module):
    ...
    def __init__(self, t, h, w, patch_t, patch_h, patch_w, dim, depth, heads, mlp_dim, dim_head=3,
        ...
        self.to_tubelet_embedding = nn.Sequential(
            ...
            nn.Linear(tubelet_dim, dim).to(self.device)
        )

        # repeat same spatial position encoding temporally
        self.pos_embedding = nn.Parameter(torch.randn(1, 1, self.nh * self.nw, dim)).repeat(1, self.nt, 1, 1).to(self.device)

        self.dropout = nn.Dropout(emb_dropout).to(self.device)

        if model == 3:
            self.transformer = FSATransformerEncoder(dim, depth, heads, dim_head, mlp_dim,
                                                     self.nt, self.nh, self.nw, dropout).to(self.device)
        elif model == 4:
    ...
    def forward(self, x):
        ...
        x,attn = self.transformer(tokens)
        x = x.mean(dim=1)
